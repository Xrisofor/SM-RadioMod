...
local renderablesTp = { "$GAME_DATA/Character/Char_Male/Animations/char_male_tp_weldtool.rend", "$CONTENT_DATA/Tools/Remote/char_radio_remote_tp_animlist.rend" }
...
function RadioRemote.loadAnimations( self )
	...
	self.tpAnimations = createTpAnimations(
        ...
        {
            idle = { "weldtool_idle" },
            use = { "weldtool_use_idle", { nextAnimation = "idle" } },
            use_err = { "weldtool_use_error", { nextAnimation = "idle" } },
    ...
    if self.tool:isLocal() then
        ...
        self.tool,
        {
            ...
            use = { "weldtool_use_into", { nextAnimation = "idle" } },
            use_err = { "weldtool_use_error", { nextAnimation = "idle" } },
...
function RadioRemote.client_onEquippedUpdate( self, primaryState, secondaryState, forceBuildActive )
    ...
    if sm.exists(result) and result.type == "body" then 
        ...
        local is_target_uuid = shape_uuid == sm.uuid.new("d0ad87eb-22ef-41fb-8c39-002e7507d7e3") or shape_uuid == sm.uuid.new("f6b6faf5-2554-49cd-aa49-23af1a6cb2a3")
